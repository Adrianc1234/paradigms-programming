...
def visualizar(df):
    ...
    for i in range(0,len(df)):
        ...
        if etiqueta == "Trabajador":
            html = f'<p>Inmuebles:{datos[0]}</p><p>viviendas:{datos[1]}</p><p>Maquinaria:{datos[2]}</p><p>Unidades de transporte:{datos[3]}</p><p>Equipo de computo:{datos[4]}</p><p>Equipo de oficina:{datos[5]}</p><p>Etiqueta:{etiqueta}</p>'
    ...
    #Abrimos el mapa
    webbrowser.open('/home/adrian/mapa.html')
